...
<Callout type="info">
  Make sure you have `yt-dlp` and `ffmpeg` installed before executing the
  commands. `yt-dlp` is a powerful command-line tool that offers additional
  features compared to the traditional YouTube downloader. `ffmpeg` is required
  for the audio extraction and conversion process.
</Callout>
